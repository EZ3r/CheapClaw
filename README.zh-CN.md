...
CheapClaw的初衷是"笔者看着自己openclaw的token账单感到心痛"。同时大部分人只是想体验一下 openclaw 而非真正的需要重度使用。
因此CheapClaw目的不是为了最小化代码量或者在不同场景下复刻 openclaw。 所以在满足OpenClaw大部分功能情况下，甚至扩展了部分功能，同时提高相同模型解决复杂问题的能力（省钱）。达成这一效果的主要不同在于单个agent loop内强弱模型的混合执行。在openclaw下，可以倚靠不同的bot，使用强模型的bot规划,然后弱模型的执行bot进行执行。但是执行bot有一个问题，如果规划在执行过程中没有像预料一样进展，那么弱模型将会崩溃。因此基于infiagent sdk的特性，在同一个agent loop中，它会阶段式的思考，并且直接截断前面全部的对话（如每 10 步或 20 步），这不仅让小模型在更短的上下文中执行，更提供了实时修正的来自强模型的计划。虽然回复“你好”和 openclaw 一样需要经过大量流程，但是当发布更复杂的任务时如“总结XX列表下500个pdf的内容到一个表格"，“完成一份至少 100 页的 EV 调研报告”，这种细粒度的混合执行会为你剩下不少钱。
...
- 除了skills外，可以给 bot添加完整的智能体系统给其调用，支持自定义的单或者多智能体系统，支持自定义的工具，通过自己或他人的垂类智能体更可靠的完成复杂任务（例如科学研究，论文撰写）。具体使用方式可以查看文档内的教程。
- 完全拆分的分层记忆，bot的调度智能体只专注于维护和用户的交互记忆，其调度的子智能体系统只维护同一个任务下对类似任务的记忆，而不会得知其他类型任务的噪声信息。
- 用户可以基于 infiagent 项目的说明，基于配置文件编辑出自己的智能体系统，并添加到对应 bot 下。（注意不要使用内置的 human_in_loop 工具给智能体系统。
