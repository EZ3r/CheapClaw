...
CheapClaw的初衷是"笔者看着自己openclaw的token账单感到心痛"。因此CheapClaw在满足OpenCLaw大部分功能情况下，甚至扩展了部分功能，同时提高相同模型解决复杂问题的能力（变相省钱）。达成这一效果的主要不同在于单个agent loop内强弱模型的混合执行。在openclaw下，可以倚靠不同的bot，使用强模型的bot规划,然后弱模型的执行bot进行执行。但是执行bot有一个问题，如果规划在执行过程中没有像预料一样进展，那么弱模型将会崩溃。因此基于infiagent sdk的特性，在同一个agent loop中，它会阶段式的思考，并且直接截断前面全部的对话（如没 10 步或 20 步），这不仅让小模型在更短的上下文中执行，更提供了实时修正的来自强模型的计划。虽然回复“你好”和 openclaw 一样需要经过大量流程，但是当发布更复杂的任务时如“总结XX列表下500个pdf的内容到一个表格"，“完成一份至少 100 页的 EV 调研报告”，这种细粒度的混合执行会为你剩下不少钱。
